var gamestage3 = function(game){
	var layer;
	var player;
}

gamestage3.prototype = {
  	create: function(){
  		this.game.physics.startSystem(Phaser.Physics.ARCADE);
  		
  		this.game.stage.backgroundColor = "#3a5963"; // 배경컬러 설정

  		var map = this.game.add.tilemap('map', 32, 32); // 타일맵 추가
  		map.addTilesetImage('tiles'); // 타일셋 이미지를 맵에 추가
  		
  		layer = map.createLayer(0); // 맵 레이어 생성
  		layer.resizeWorld(); // 레이어 페이저 화면에 맞게 리사이저
  		
  		map.setCollision([0]); // 충돌 범위 설정
  		map.setCollision([1]);
  		
  		var smile = this.game.add.sprite(300, 300, 'smile');
  		
  		var nextButton = this.game.add.button(200,200,"button",this.moveNextStage3,this);
  		nextButton.anchor.setTo(0.5,0.5);
  		
  		player = this.game.add.sprite(300, this.game.world.height-300, 'dude');
